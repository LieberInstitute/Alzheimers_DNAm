...
blockPlot = function(cset, blocks450, coi, N=10,
	...
	# cancer blocks
	if(showCancerPanel) {
		load("/users/ajaffe/Lieber/Projects/450k/devPaper/cancer_blocks_hansen.rda")
		genome(blocks)="hg19"
		cancerBlocks = blocks
	}
...
dmrPlot = function(regions, p, chr, pos, cluster, genes, coi,
	...
	if(build == "hg18") {
		cpg.cur = read.table("http://www.haowulab.org/software/makeCGI/model-based-cpg-islands-hg18.txt",
			header = TRUE, as.is=TRUE)
		library("BSgenome.Hsapiens.UCSC.hg18")
	}
	
	if(build == "hg19") {
		cpg.cur = read.table("http://www.haowulab.org/software/makeCGI/model-based-cpg-islands-hg19.txt", header = TRUE, as.is=TRUE)
		library("BSgenome.Hsapiens.UCSC.hg19")
	}
	
	if(build == "mm9") {
		cpg.cur = read.table("http://www.haowulab.org/software/makeCGI/model-based-cpg-islands-mm9.txt",
			header = TRUE, as.is=TRUE)
		library("BSgenome.Mmusculus.UCSC.mm9")
	}
	if(build == "hg38") {
		cpg.cur = read.table("/dcl01/lieber/ajaffe/Steve/meth450k_annotation_hg38/makeCGI/CGI-Hsapiens.txt",
			header = TRUE, as.is=TRUE)
	...
	anno = annotateRegions(gr2, genomicState)$annotationList
