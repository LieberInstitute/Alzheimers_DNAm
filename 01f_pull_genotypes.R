...
newbfile = "/dcl01/lieber/ajaffe/Steve/Alz/Paper/genotype_data/Alz_meth450k_Genotypes_n377_maf005_geno10_hwe1e6"
forceKeep_bfile = "/dcl01/lieber/ajaffe/Steve/Alz/Paper/genotype_data/Alz_meth450k_Genotypes_n377_forceKeep"
...
#############
## save #####
save(mds, snp, snpMap, compress=TRUE, file = "Alz_meth450k_Genotypes_n377.rda")

##### Add MDS components to pd object
setwd('../')
load('genotype_data/Alz_meth450k_Genotypes_n377.rda')
load('rdas/cleanSamples_n377_processed_data_postfiltered.rda')
...
ggsave(pc5_plot, filename='qc/pairwise_top5_snpPCs_array.pdf',height=40,width=40 )

pc5_plot = ggpairs(data=as.data.frame(pd[,c('snpPC1','snpPC2','snpPC3','snpPC4','snpPC5','Race') ]) ,
		columns=1:5, # data.frame with variables
        title="First 5 Genotype MDS Components",
		mapping = aes(color = `Race`) )
ggsave(pc5_plot, filename='qc/pairwise_top5_snpPCs_race.pdf',height=40,width=40 )
