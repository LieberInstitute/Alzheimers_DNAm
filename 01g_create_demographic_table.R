...
a <- ggplot(data=dat, aes(x=braak, y=Age, fill=DxOrdinal)) +
		...
		geom_point(aes(col=`DxOrdinal`),position = position_jitterdodge(jitter.width=0.3,dodge.width=.85)) + 	scale_fill_manual(values=c("black",'#325630',"#ab1323" ) ) +
		scale_colour_manual(values=c("black",'#325630',"#ab1323" ) ) + 		
...
c <- ggplot(data=dat, aes(x=DxOrdinal, y=Age,fill=DxOrdinal,col=DxOrdinal) ) +
		...
		scale_fill_manual(values=c("black",'#325630',"#ab1323" ) ) +
		scale_colour_manual(values=c("black",'#325630',"#ab1323" ) ) 
...
d <- ggplot(data=dat, aes(x=DxOrdinal, y=brain_weight_gram,fill=DxOrdinal,col=DxOrdinal) ) +
		...
		scale_fill_manual(values=c(Control="black",`Asymptomatic AD`='#325630',`Symptomatic AD`="#ab1323" ) ) +
		scale_colour_manual(values=c(Control="black",`Asymptomatic AD`='#325630',`Symptomatic AD`="#ab1323" ) )
...
lay <- rbind(c(1,1,2,2),
             c(3,4,5,6))
demo_boxplots <- arrangeGrob(a, b, c,d,e,f, layout_matrix =lay) 
