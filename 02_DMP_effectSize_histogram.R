...
## load data
load('/dcl01/lieber/ajaffe/Steve/Alz/rdas/allRegion_mergedStats_DMP_analysis_dupCor.rda',verbose=T)
...
ggsave(cpg_main, file='/dcl01/lieber/ajaffe/Steve/Alz/plots/maineffect_dmp_enrichment_in_cpg.pdf',height=14,width=10 )
...
pdf('/dcl01/lieber/ajaffe/Steve/Alz/plots/main_effect_subsetModel_unadjusted_vs._adjusted_cell_type_senstivity.pdf')
...
pdf('/dcl01/lieber/ajaffe/Steve/Alz/plots/main_effect_subsetModel_primary_vs_ordinal_secondary_model_tstatistics.pdf')
par(mar=c(5,6,2,2))
plot(primaryStats[,'ALL_subset_mainEffect_t'], 
...
table(primary=primaryStats[,'ALL_subset_mainEffect_adj.P.Val']<0.05, 
	 ...
	 ### APOE Sensitivity
load('/dcl01/lieber/ajaffe/Steve/Alz/rdas/APOE4_sensitivity_allRegion_mergedStats_DMP_analysis_dupCor.rda',verbose=T)

cor.test(primaryStats[,'ALL_subset_mainEffect_t'], APOE_allRegionStats[rownames(primaryStats),"ALL_subset_mainEffect_t"])

png('/dcl01/lieber/ajaffe/Steve/Alz/plots/main_effect_subsetModel_primary_vs_APOE_Sensitivitiy_model_tstatistics.png')
par(mar=c(5,6,2,2))
plot(primaryStats[,'ALL_subset_mainEffect_t'], 
...
histo_gene <- ggplot() + geom_histogram(data = allRegion_mergedStats[which(allRegion_mergedStats$ALL_subset_mainEffect_logFC < 0 & allRegion_mergedStats$ALL_subset_mainEffect_adj.P.Val <0.05), ],
		   ...
		   theme(axis.title=element_text(size=50),
				 ...
				 panel.grid.minor = element_blank()) +
scale_x_continuous(limits = c(-0.2, 0.2))  + scale_y_continuous(limits = c(0, 300), expand = c(0, 0)) + 
 annotate("text", x = -0.10, y = 200, label = paste0("620 \nhypomethylated"),size=8) +
 annotate("text", x = 0.135, y = 200, label = paste0("874 \nhypermethylated"),size=8)
 
ggsave(histo_gene, file = '/dcl01/lieber/ajaffe/Steve/Alz/plots/Histogram_of_DNAm_Percent_Change_MainSubsetDupCorModel.pdf',height=10,width=10)
