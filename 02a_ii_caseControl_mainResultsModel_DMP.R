#qsub -l bluejay,mf=60G,h_vmem=60G,h_fsize=200G,h_stack=256M -cwd -b y -M stephensemick@gmail.com -o log -e log R CMD BATCH --no-save 02a_ii_caseControl_mainResultsModel_DMP.R
...
## Interaction effect
mod <- model.matrix(~DxOrdinal+ negControl_PC1 + negControl_PC2 + Age+Sex + snpPC1 + Region + DxOrdinal:Region, data = pd)
...
save(allRegion_mergedStats, file='/dcl01/lieber/ajaffe/Steve/Alz/rdas/allRegion_mergedStats_DMP_analysis_dupCor.rda' )		
