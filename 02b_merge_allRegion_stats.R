...
save(allStats, file='rdas/caseControl_DMC_allRegion.rda' )

### Cut FDR significant (<5%) main effect and interaction effect CpGs for supplemental tables
write.csv(allStats[allStats$Primary_subset_mainEffect_adj.P.Val<0.05, ],row.names=F,file='csvs/SupplementalTable_mainEffect_cpgs_fdr05_dupcor.csv')
write.csv(allStats[allStats$Primary_subset_interactionEffect_adj.P.Val<0.05,],row.names=F,file='csvs/SupplementalTable_interaction_cpgs_fdr05_dupcor.csv')
##### Tidying up the stats object
tidyMergedStats= allStats %>%
  gather(key, value, 40:ncol(allStats))
...
save(allRegion_tidyStats, file ='rdas/tidyStats_caseControl_DMC_allRegion.rda')
##
...
write.csv(allRegion_DMP_MetaStats, 'csvs/allRegion_caseControl_DMP_Results_Metastats_FDR05.csv',row.names=F)


## 
load('rdas/merged_DMP_regionSpecific_caseControl_stats.rda')
write.csv(regionSpecific_mergedStats[rowSums(regionSpecific_mergedStats[,grep("subset_NoAdj_adj.P.Val", colnames(regionSpecific_mergedStats),value=T)]<0.05)>0,],file='csvs/SupplementalTable_singleRegion_caseControl_DMP_Results_FDR05_subset_notCellTypeAdjusted.csv',row.names=FALSE)


tidyMergedStats= regionSpecific_mergedStats %>%
  gather(key, value, 40:ncol(regionSpecific_mergedStats))
...
save(singleRegion_tidyStats, file ='rdas/tidyStats_caseControl_DMC_singleRegion.rda')
...
write.csv(singleRegion_DMP_MetaStats, 'csvs/singleRegion_caseControl_DMP_Results_Metastats.csv',row.names=F)
